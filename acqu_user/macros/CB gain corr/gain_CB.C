...
void gain_CB()
{
	...
	string path = "/home/adlarson/data2014.07/CaLib/it9newcl/";

	TString pathtohist = "CaLib_CB_IM";
	ostringstream s;
	
	ofstream prtofile;
	prtofile.open("CB_gain_June25_2015.dat");
	ofstream prtofileav;
...
// Creating 720 TH2F histograms which will contain the IM(gg) All versus detector nr.
	...
	TH2F* detnr_allruns	= new TH2F("detnr_allruns", "detnr_allruns", 720, 0, 720, 300, 120., 150.); 
	TH2F* sumAll = new TH2F("sumAll", "sumAll", 500, 0., 1000., 720, 0, 720);
	...
	{
		TH1F* prpi0av = (TH1F*)sumAll->ProjectionX( "IM(#gamma#gamma) (MeV)", ibn+1, ibn+1)->Clone();
	// insert the average over the full peak projection here
		if( prpi0av->Integral() == 0 ) 
		{
			detnr_allruns->Fill( ibn , 145.00 );
		}
		else
		{
			Double_t help[2];
	...
	allruns->SaveAs("allruns_vs_CBdetel.eps");
	delete allruns;
...
//					prpi0av->Rebin(2);
	...
	for (int i = 0; i < nCanvas; i++)
	{
		...
		CCAC[i]->SaveAs(Form("it4_%02d.eps", i));
	}
	for(int i = 0; i < nCanvas; i++)
		delete CCAC[i];
	delete CCAC;
